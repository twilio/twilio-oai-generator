...
function generate() {
  ...
  for api_spec in $files_regex; do
    ...
    if [ "$1" !=  "twilio-node" ]  && [ "$1" !=  "twilio-ruby" ]  && [ "$1" !=  "twilio-python" ] && [ "$1" !=  "twilio-php" ]  && [[ $api_spec == "examples/spec/twilio_iam_organizations_v1.yaml" || $api_spec == "examples/spec/twilio_iam_organizations_versionless.json" ]]; then
        continue
    fi
