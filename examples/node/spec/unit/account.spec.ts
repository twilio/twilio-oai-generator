...
describe("account", () => {
  ...
  it("should get account pages in between dates", () => {
    ...
    return twilio.api.v2010.accounts.page(params)
        .then((accountPage) => expect(accountPage.nextPageUrl).toEqual('https://api.twilio.com/2010-04-01/Accounts.json?FriendlyName=friendly_name&Status=active&PageSize=50&Page=50'))
        .then(() => scope.done());
  });
});
